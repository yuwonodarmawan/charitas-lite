...
class wplook_posts_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
	 		'wplook_posts_widget',
			'WPlook Posts',
			array( 'description' => __( 'A widget for displaying latest Posts', 'charitas' ), )
		);
	}
	...
	public function form( $instance ) {
		...
		?>
			...
			<p>
				<label for="<?php echo $this->get_field_id('read_more_link'); ?>"> <?php _e('URL to all Posts:', 'charitas'); ?> </label>
				...
				<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;"> <?php _e('View all posts URL', 'charitas'); ?></p>
			</p>

			<p>
				<label for="<?php echo $this->get_field_id('size'); ?>">
					<?php _e('Clear after?', 'charitas'); ?>
					<br />
				</label>
				<select id="<?php echo $this->get_field_id('clear_after'); ?>" name="<?php echo $this->get_field_name('clear_after'); ?>">
					<option value="0" <?php selected( '0', $clear_after ); ?>><?php _e('Yes', 'charitas'); ?></option>
					<option value="1" <?php selected( '1', $clear_after ); ?>><?php _e('No', 'charitas'); ?></option>
				</select>
				<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;"> <?php _e('Clear after if you want to add one more widged after this widget', 'charitas'); ?></p>
				...
				<aside class="widget WPlookPosts">	
					<div class="widget-title">
						...
						<?php if ( $read_more_link != "") { ?>
							<div class="viewall fright"><a href="<?php echo $read_more_link; ?>" class="radius" title="<?php _e('View all posts', 'charitas'); ?>"><?php _e('view all', 'charitas'); ?></a></div>
						<?php } ?>
